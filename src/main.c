...
static void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "  %s <source.x> --emit-c     Only emit C code\n"
        "  %s <source.x] --ast        Print AST\n"
        "  %s <source.x> --tokens     Print tokens\n",
        prog, prog, prog, prog, prog);
}

int main(int argc, char **argv) {
    if (argc == 2 && strcmp(argv[1], "--version") == 0) {
        printf("XLang Compiler v1.0\n");
        return 0;
    }
    ...
    if (emit_c) {
        ...
        char cmd[1024];
        snprintf(cmd, sizeof(cmd), "gcc -o \"%s\" \"%s\" -lm 2>&1", out_name, c_path);
        int ret = system(cmd);
        remove(c_path);
